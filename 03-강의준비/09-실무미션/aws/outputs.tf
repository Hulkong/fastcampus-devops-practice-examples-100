...
output "configure_argocd" {
  ...
  value       = <<-EOT
    ...
    argocd login --port-forward --username admin --password $(aws secretsmanager get-secret-value --secret-id ${local.name}/${local.env}/argocd --region ${local.region} --output json | jq -r .SecretString | jq -r .ADMIN_PASSWORD)

    echo "ArgoCD Username: admin"
    echo "ArgoCD Password: $(aws secretsmanager get-secret-value --secret-id ${local.name}/${local.env}/argocd --region ${local.region} --output json | jq -r .SecretString | jq -r .ADMIN_PASSWORD)"
