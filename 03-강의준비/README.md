...
|챕터명|클립명|실습자료명|
|---|---|---|
|사전준비|CH22_01. 사전준비|01-prepare|
|테라폼에서 발생하는 이슈를 대응해 본다.|CH23_03. 시나리오 설명 및 실습|02-senario|
|AWS 서비스 쿼타의 문제를 발생시키고, 해결해 본다.|CH25_03. 시나리오 설명 및 실습|03-senario|
|컨테이너를 운영 중인 인스턴스의 볼륨 용량 초과 이슈를 대응해 본다.|CH26_03. 시나리오 설명 및 실습|04-senario|
|Github pr 프로세스를 적용하여 서비스의 완성도와 개발생산성을 높여본다.|CH27_03. 시나리오 설명 및 실습|05-senario|
|비용절감을 위하여 람다를 이용하여 유휴상태인 RDS 사용정지하는 시나리오를 진행해 본다.|CH28_01. 비용절감을 위하여 람다를 이용하여 유휴상태인 RDS 사용정지하는 시나리오 설명 및 실습|06-senario|
|gitops-bridge를 사용하여 한 큐에 EKS Best Practice를 적용한 인프라, 애드온, 서비스 구축해 본다.|CH29_03. 시나리오 설명 및 실습|07-senario|
|github의 모든 리소스를 테라폼으로 관리|CH30_02. 시나리오 설명 및 실습|08-senario|
|효율적인 인프라 관리를 위해서 Git과 IaC를 이용하여 운영한다. 또한, 예외처리를 진행해본다.(실무미션)|CH31_01. 효율적인 인프라 관리를 위해서 Git과 IaC를 이용하여 운영한다. 또한, 예외처리를 진행해본다.|08-실무미션|
...
|Github|소프트웨어 개발 및 버전 관리를 위한 웹 기반의 Git 저장소|-|-|
...
> EKS는 03-강의준비/main.tf 파일을 기반으로 테라폼으로 프로비저닝 할 수 있습니다.
...
terraform -chdir=03-강의준비 init
terraform -chdir=03-강의준비 plan
terraform -chdir=03-강의준비 apply
```
...
aws eks --region us-west-2 update-kubeconfig --name part03
...
kubectl config use-context arn:aws:eks:us-west-2:{AWS Account ID}:cluster/part03
```
